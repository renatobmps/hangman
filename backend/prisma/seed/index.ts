...
async function migrate() {
  async function getUsers(): Promise<{ rows: Array<User> }> {
    try {
      const users = await oldDatabase.$queryRaw<
        Array<User>
      >`SELECT x.* FROM public."Users" AS x`;
  ...
  async function getWords(): Promise<{ rows: Array<Word> }> {
    try {
      const words = await oldDatabase.$queryRaw<
        Array<Word>
      >`SELECT x.* FROM public."Words" AS x`;
  ...
  async function getGames(): Promise<{ rows: Array<Game> }> {
    try {
      const games = await oldDatabase.$queryRaw<
        Array<Game>
      >`SELECT x.* FROM public."UserWords" AS x`;
  ...
  async function getUsedLetters(): Promise<{ rows: Array<Letter> }> {
    try {
      const letters = await oldDatabase.$queryRaw<
        Array<Letter>
      >`SELECT x.* FROM public."TriedLetters" As x`;
  ...
  const [Users, Words, Games, UsedLetters] = await Promise.all([
    getUsers(),
    getWords(),
    getGames(),
    getUsedLetters(),
  ...
  async function migrateHint() {
    const allHints = [
      ...new Set(
        Words.rows
          .filter((word) => typeof word.hint !== "undefined")
          .map((word) => word.hint.trim()),
      ),
    ];

    const targetHints = await prisma.hint
      .findMany({
        select: { text: true },
      })
      .then((data) => data.map((data) => data.text));

    const remainingHints = allHints.filter(
      (hint) => !targetHints.includes(hint),
    );

    await Promise.all(
      remainingHints.map((hint) =>
        prisma.hint.create({
          data: {
  ...
  async function migrateWords() {
    const allWords = [
      ...new Set(
        Words.rows
          .filter((word) => typeof word.word !== "undefined")
          .map((word) => ({
            word: word.word.trim(),
    ...
    const targetWords = await prisma.word
      .findMany({
        select: { text: true },
      })
      .then((data) => data.map((data) => data.text));
    ...
    const remainingWords = allWords.filter(
      (word) => !targetWords.includes(word.word),
    );
  ...
  async function migrateWordsAndHintRelation() {
    ...
    for (const word of allWords) {
      const wordId = await prisma.word
        .findFirst({
          select: { id: true },
      ...
      const hintId = await prisma.hint
        .findFirst({
          select: { id: true },
      ...
      if (wordId && hintId) {
        allWords[
          allWords.findIndex(
            (currentWord) =>
              currentWord.word === word.word && currentWord.hint === word.hint,
          )
        ] = {
          ...word,
          wordId,
    ...
    const withRelation = allWords.filter(
      (w) => typeof w.hintId !== "undefined" && typeof w.wordId !== "undefined",
    );

    const targetRelationships = await prisma.hintsWords.findMany({
      select: { hint_id: true, word_id: true },
    });
    const remainingRelationships = withRelation.filter(
      (word) =>
        !targetRelationships.some(
          (relationship) =>
            relationship.hint_id === word.hintId &&
            relationship.word_id === word.wordId,
        ),
    );

    await Promise.all(
      remainingRelationships.map((relationship) =>
        prisma.hintsWords.create({
          data: {
  ...
  async function migrateUsers() {
    ...
    const targetUsers = await prisma.user
      .findMany({
    ...
    const remainingUsers = allUsers.filter(
      (user) => !targetUsers.includes(user.name),
    );
  ...
  async function migrateGame() {
    const allGames = Games.rows.map((data) => ({
      ...
      attempts:
        data.initialLife -
        UsedLetters.rows.filter(
          (letter) =>
            letter.idUserWords === data.id && letter.correct === false,
        ).length,
      isBeat:
        data.initialLife -
          UsedLetters.rows.filter(
      ...
      user: replaceSpaces(
        removeAccents(
          Users.rows
            .find((user) => user.id === data.idUsers)
            ?.name.trim()
            .toLowerCase() ?? "",
        ),
      ),
      word:
        Words.rows.find((word) => word.id === data.idWords)?.word.trim() ?? "",
      id: data.id,
    }));
    ...
    for (let i = 0; i < allGames.length; i++) {
      ...
      const [user, word] = await Promise.all([
        ...
        prisma.word.findFirst({
          select: { id: true },
          where: { text: data.word.toLowerCase().trim() },
        }),
      ]);
    ...
    const targetGames = await prisma.game.findMany({
      select: { user_id: true, word_id: true },
    });
    ...
    const remainingGames = allGames.filter(
      (game) =>
        !targetGames.some(
          (targetGame) =>
            targetGame.user_id === game.user &&
            targetGame.word_id === game.word,
        ),
    );
  ...
  async function migrateGameLetters() {
    const allLetters = UsedLetters.rows.map((data) => {
      ...
      return {
        ...
        user: replaceSpaces(
          removeAccents(
            Users.rows
              .find((user) => user.id === game?.idUsers)
              ?.name.trim()
              .toLowerCase(),
          ),
        ),
    ...
    for (let i = 0; i < allLetters.length; i++) {
      ...
      const game = await prisma.game.findFirst({
        select: { id: true },
        where: { user_id: user?.id, word_id: word?.id },
      });
    ...
    const targetLetters = await prisma.gameLetter
      .findMany({ select: { game_id: true } })
      .then((data) => data.map((data) => data.game_id));

    const remainingLetters = allLetters.filter(
      (letter) => !targetLetters.includes(letter.game!),
    );

    await Promise.all(
      remainingLetters
        .filter((data) => typeof data.game !== "undefined")
        .map((letter) => {
  ...
  await Promise.all([migrateHint(), migrateWords(), migrateUsers()]);

  await Promise.all([migrateWordsAndHintRelation(), migrateGame()]);

  await Promise.all([migrateGameLetters()]);
}
