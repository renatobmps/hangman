...
describe("AddWordsRepository unit", () => {
  ...
  it("should to run word create", async () => {
    ...
    deepEqual(databaseMock.word.create.mock.callCount(), 1);
    deepEqual(databaseMock.word.create.mock.calls[0].arguments.at(0), {
  ...
  it("should to disconnect", async () => {
    ...
    deepEqual(databaseMock.$disconnect.mock.callCount(), 1);
  });

  it("should to create without description", async () => {
    await repository.execute(ADD_WORD_CREATE_DATA_WITHOUT_DESC);
  ...
  it("should to create without activated", async () => {
    await repository.execute(ADD_WORD_CREATE_DATA_WITHOUT_ACTIVE);
  ...
  it("should to create with only required data", async () => {
    await repository.execute(ADD_WORD_CREATE_DATA_REQUIRED);
