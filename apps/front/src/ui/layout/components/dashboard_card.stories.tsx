...
const S = {
  DashboardCard: styled(Link)<{ $checked?: boolean }>`
    ...
    display: grid;
    grid-template-columns: var(--grid_left_proportion) calc(
        100% - var(--grid_left_proportion)
      );
    height: auto;
  ...
  DashboardCardAttempts: styled.span<{ $total: number }>`
    ...
    ${({ $total }) => {
      if ($total <= 0) {
        return css`
          --card_attempts_c: ${({ theme }) => theme.palette.negative[100]};
        `;
      } else if ($total <= 2) {
        return css`
          --card_attempts_c: ${({ theme }) => theme.palette.negative[300]};
        `;
      } else if ($total <= 4) {
        return css`
          --card_attempts_c: ${({ theme }) => theme.palette.negative[500]};
        `;
      }
    }}
...
const LayoutComponents = {
  DashboardCard: ({
    ...
    <S.DashboardCard
      $checked={attempts > 0 && win}
      href={`/dashboard/games/${game_id}`}
    >
      <S.DashboardCardWord>{word}</S.DashboardCardWord>
      <S.DashboardCardAttempts $total={attempts}>
        {attempts}
      </S.DashboardCardAttempts>
