...
export default function Login() {
  ...
  const statusRequest = async () => {
    const request = await fetch('/api/v1/status').catch((err) => {
      throw new Error(err?.message ?? 'Falha ao requisitar dados');
    });

    if (!request?.ok) {
      throw new Error(`Servidor respondeu com erro ${request.status}`);
    }

    const data = await request.json().catch((err) => {
      throw new Error(err?.message ?? 'Falha ao parsear os dados');
    });
  ...
  useEffect(() => {
    statusRequest()
      ...
      .catch((err) => {
        alert(err?.message ?? 'Servidor indisponível');
        setStatus(null);
      });
  }, []);
